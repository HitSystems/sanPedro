...
export class PromocionesClass {
    getPromocionesUgly(database: string, codigoCliente: number) {
        return recHit(database, `SELECT Id as _id, Di as fechaInicio, Df as fechaFinal, D_Producte as principal, D_Quantitat as cantidadPrincipal, S_Producte as secundario, S_Quantitat as cantidadSecundario, S_Preu as precioFinal FROM ProductesPromocionats WHERE Client = ${codigoCliente}`).then((res: IResult<any>) => {
            if (res) {
