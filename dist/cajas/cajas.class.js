...
function comprobarDatosCaja(caja) {
    ...
    if (caja.primerTicket == undefined || caja.primerTicket == null || typeof caja.primerTicket != "number") {
        return { error: true, mensaje: 'primerTicket no es correcto' };
    }
...
class CajasClass {
    async insertarCajas(parametros, infoCaja, server) {
        if (datosCorrectosParametros(parametros).error == false) {
            if (comprobarDatosCaja(infoCaja).error == false) {
                const fechaInicio = (0, fechas_1.fechaParaSqlServer)(new Date(infoCaja.inicioTime));
                const fechaFinal = (0, fechas_1.fechaParaSqlServer)(new Date(infoCaja.finalTime));
                let sqlZGJ = '';
                let sqlW = '';
                let sqlWi = '';
                let sqlO = '';
            ...
            `;
                ...
                try {
                    resSqlRepeticion = await (0, mssql_1.recHit)('hit', sqlRepeticion);
                }
                catch (err) {
                    console.log(err);
                    errorSalir = true;
                ...
                if (errorSalir == false) {
                    if (resSqlRepeticion.recordset.length == 1) {
                        if (resSqlRepeticion.recordset[0].resultado == 'OK') {
                            (0, mssql_1.recHit)(parametros.database, sqlCompleta).then((aux) => {
...
exports.cajasInstance = cajasInstance;
//# sourceMappingURL=cajas.class.js.map